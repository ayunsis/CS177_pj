...
plt.figure(figsize=(12, 7))
sns.histplot(core2016_df['gap'], bins=30, color=core_color, label='GroundTruth Gap', kde=True, stat='density', alpha=0.6, edgecolor='black')
sns.histplot(sfcnn_df['gap'], bins=30, color=sfcnn_color, label='Coreset Sfcnn Gap', kde=True, stat='density', alpha=0.6, edgecolor='black')
plt.title('Distribution of Absolute Gaps\n(Ground Truth and Coreset Sfcnn)', fontsize=16)
...
plt.ylabel('Absolute Gap', fontsize=13)
plt.xticks(ticks=[0, 1], labels=['GroundTruth', 'Coreset SFCNN'], fontsize=12)
plt.yticks(fontsize=11)
plt.grid(True, linestyle=':', alpha=0.8)
# plt.legend(title='Dataset', fontsize=10, title_fontsize=11) # Legend might be redundant with explicit x-axis labels
plt.tight_layout()
