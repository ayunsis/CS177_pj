...
import random
import pickle
import h5py  # Added for efficient storage

# Converts the protein-ligand complexes into 4D tensor. 
class Feature_extractor():
    def __init__(self):
        self.atom_codes = {}
        # 'others' includes metal atoms and B atom. There are no B atoms on training and test sets. 
...
SAMPLE_PATH = r'data/sample_refined'
...
SAMPLE_LABELS = r'data/sample_train_hdf5/sample_label.h5'
SAMPLE_GRIDS = r'data/sample_train_hdf5/sample_grids.h5'
...
# Save the label data of training and test set to HDF5
...
with h5py.File(TRAIN_GRIDS, 'w') as h5f:
    ...
    grid_shape = sample_grid.shape
    dset = h5f.create_dataset('train_grids', shape=(num_train,)+grid_shape, dtype=np.float32)
    for idx, mols in enumerate(train_complexes):
        print(f"Processing training complex {idx+1}/{num_train}...")
        coords1, features1 = Feature.get_features(mols[0],1)
        coords2, features2 = Feature.get_features(mols[1],0)
        center=(np.max(coords2,axis=0)+np.min(coords2,axis=0))/2
        coords=np.concatenate([coords1,coords2],axis = 0)
